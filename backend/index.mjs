...
function isValidCURP(curp) {
  ...
  return (
    !curp || /^[A-Z]{4}\d{6}[HM][A-Z]{5}[A-Z0-9]{2}$/.test(curp.toUpperCase())
  );
}
...
app.post("/api/login", async (req, res) => {
  ...
  try {
    ...
    // Buscar datos del docente si existe
    let docente = null;
    if (user.id_empleado) {
      const [docRows] = await conn.execute(
        "SELECT nombre, apellido_paterno, apellido_materno FROM docentes WHERE id_empleado = ?",
        [user.id_empleado]
      );
      if (docRows.length > 0) {
        docente = docRows[0];
    ...
    res.json({
      ...
      email: user.email,
      ...docente, // Si existe, agrega nombre, apellido_paterno, apellido_materno
    });
  } catch (err) {
  ...
  try {
    ...
    if (search) {
      ...
      where =
        "WHERE " +
        searchFields
          .map((f) => (f === "id_empleado" ? `${f} = ?` : `${f} LIKE ?`))
          .join(" OR ");
      params = searchFields.map((f) =>
        f === "id_empleado" && !isNaN(Number(search))
          ? Number(search)
          : `%${search}%`
      );
    }
...
app.post("/api/docentes", async (req, res) => {
  ...
  if (!isValidName(apellido_paterno)) {
    return res
      ...
      .json({
        error:
          "El apellido paterno solo debe contener letras y espacios, mínimo 2 caracteres.",
      });
  }
  if (!isValidName(apellido_materno)) {
    return res
      ...
      .json({
        error:
          "El apellido materno solo debe contener letras y espacios, mínimo 2 caracteres.",
      });
  }
  if (!isValidSexo(sexo)) {
    return res
      .status(400)
      .json({ error: "El sexo seleccionado no es válido." });
  }
  ...
  if (!isValidPhone(telefono)) {
    return res
      .status(400)
      .json({ error: "El teléfono no tiene un formato válido." });
  }
  if (rfc && !isValidRFC(rfc)) {
    return res
      .status(400)
      .json({ error: "El RFC no tiene un formato válido." });
  }
  if (curp && !isValidCURP(curp)) {
    return res
      .status(400)
      .json({ error: "El CURP no tiene un formato válido." });
  }
  if (tipo_contratacion && !isValidTipoContratacion(tipo_contratacion)) {
    return res
      .status(400)
      .json({ error: "El tipo de contratación seleccionado no es válido." });
  }
  if (tipo_plaza && !isValidTipoPlaza(tipo_plaza)) {
    return res
      .status(400)
      .json({ error: "El tipo de plaza seleccionado no es válido." });
  }

  // Validar unicidad de correo
  try {
    const [existing] = await pool.query(
      "SELECT 1 FROM docentes WHERE correo = ?",
      [correo]
    );
...
app.put("/api/docentes/:id", async (req, res) => {
  ...
  try {
    const [existing] = await pool.query(
      "SELECT 1 FROM docentes WHERE correo = ? AND id_empleado != ?",
      [correo, id]
    );
    if (existing.length > 0) {
      return res.status(400).json({ error: "El correo ya está registrado." });
...
app.delete("/api/docentes/:id", async (req, res) => {
  const { id } = req.params;
  try {
    await pool.execute(`DELETE FROM docentes WHERE id_empleado = ?`, [id]);
